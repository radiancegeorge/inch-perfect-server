...
export default function Component({ data }) {
  ...
  const setDelivered = () => {
    axios({
      url: `${url}orders/order_status?delivered=${data.id}`,
      headers,
      method: "patch",
    }).then((response) => {
  ...
  return (
    <Fragment>
      ...
      {details && (
        <div class="full_order" onClick={() => setShowDetails(false)}>
          <div class="order_content" onClick={(e) => e.stopPropagation()}>
            <div className="top">
              <label>
                Order Id: <div className="order_id">{data.id}</div>
              </label>
              <label>
                Customer's Name:
                <div className="name">{`${data.first_name} ${data.last_name}`}</div>
              ...
              <label>
                Payment status:
                <div>{data.method === "AUTOMATIC" ? "PAID" : "Check mail"}</div>
              </label>
              <label>
                Location:
                <div>{`${data.street},${data.town},${data.state},${data.country}`}</div>
              </label>
              <label>
                Processing status:
                <div>
                  {data.delivered === "1" ? "In Progress" : "Processed"}
                </div>
              </label>
            </div>
            <hr />
